...
import com.laoxiao.transformer.mr.TransformerOutputFormat;
import com.laoxiao.util.JdbcManager;
import com.laoxiao.util.TimeUtil;
import com.google.common.collect.Lists;

/**
 * 计算新增用户入口类<新增用户和活跃用户都是比较关心的指标> by Ethan
 * 
 * @author 肖斌
 *
 */
public class NewInstallUserRunner implements Tool {
    ...
    public void setConf(Configuration conf) {
    	/**
    	 * 配置db 链接 和 数据插入sql 通过xml 文件，起格式与hadoop 应一致 by Ethan
    	 */
...
//        job.setInputFormatClass(KeyValueTextInputFormat.class);
     ...
     * @param conf
     * 
     * 因为时间有点凌乱，这个地方不能得到 历史新增加的总数 by Ethan 
     * 
     */
    private void calculateTotalUsers(Configuration conf) {
        Connection conn = null;
        PreparedStatement pstmt = null; // 预编译sql
        ResultSet rs = null;

        try {
            long date = TimeUtil.parseString2Long(conf.get(GlobalConstants.RUNNING_DATE_PARAMES));
            //processArgs function 对这个值做了处理 
            System.out.println(conf.get(GlobalConstants.RUNNING_DATE_PARAMES));
    ...
    private void processArgs(Configuration conf, String[] args) {
        ...
        // 问题就是出在这里了 ，这个MapReduer 不能处理历史数据 ，只能处理昨天的数据 
    ...
    private List<Scan> initScans(Job job) {
        ...
        //因为生成数据不再定义的范围内，所以暂时注释掉下面两行  by Ethan 2016-07-02
...
/***
 * 1.WARN - Output Path is null in commitJob()
 * 
 * 
 * 
 * 
